...
Playfair is used in the logo name and for main headings on the [website](https://www.portsmouthclimateaction.org.uk/). None of the logo variants require the Playfair font, as the letterforms are all converted to outline. (But the text of the name is retained as editable text in a dedicated layer in `/src/pca-logo-master.ai`).

Get **Playfair Display Black**:

- From the designer, Claus Eggers Sørensen: [Playfair 2.0](https://forthehearts.net/playfair2_0/)
- [Playfair from Google Fonts](https://fonts.google.com/specimen/Playfair+Display?query=playfair+display)
- [Playfair from Adobe Fonts](https://fonts.adobe.com/fonts/playfair)

On the website, body text is set in the **Open Sans** font family:

- [Open Sans from Google Fonts](https://fonts.google.com/specimen/Open+Sans?query=open+sans)
- [Open Sans from Adobe Fonts](https://fonts.adobe.com/fonts/open-sans)
...
- `for-circle`: symbol only, for use in a circle:
    - The assumption is that the social media platform where the logo is deployed will apply a circular mask to it
- `linear`: symbol + name in a single line
- `2-line`: symbol + name in two lines
...
- `svg` is the preferred format for the sharpest results online, when that format is supported, but this will not be the case for social media platforms so…
